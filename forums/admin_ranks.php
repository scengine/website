...
if (isset($_POST['add_rank']))
{
	...
	if ($rank == '')
		message('Vous devez saisir un titre au rang.');

	if (!@preg_match('#^\d+$#', $min_posts))
		message('Le nombre minimum de messages doit être un entier positif.');
	...
	if ($db->num_rows($result))
		message('Il y a déjà un rang avec un nombre minimum de '.$min_posts.'messages.');
	...
	redirect('admin_ranks.php', 'Rang ajouté. Redirection ...');
}
...
else if (isset($_POST['update']))
{
	...
	redirect('admin_ranks.php', 'Rangs modifiés. Redirection ...');
}
...
else if (isset($_POST['remove']))
{
	...
	redirect('admin_ranks.php', 'Rang supprimé. Redirection ...');
}


$page_title = pun_htmlspecialchars($pun_config['o_board_title']).' / Admin / Rangs';
	...
	<div class="blockform">
		...
		<div class="box">
			<form id="ranks" method="post" action="admin_ranks.php?action=foo">
						...
						<div class="infldset">
							...
							<tbody>
								<tr>
									...
									<td><input type="submit" name="add_rank" value=" Ajouter " tabindex="3" /></td>
								</tr>
							</tbody>
							</table>
						</div>
				...
				<div class="inform">
					<fieldset>
						<legend>Modifier/supprimer les rangs</legend>
						<div class="infldset">
<?php
...
?>
							...
							<thead>
								<tr>
									<th class="tcl" scope="col"><strong>Titre du rang</strong></th>
									<th class="tc2" scope="col"><strong>Nombre minimum de messages</strong></th>
...
<?php
	...
	while ($cur_rank = $db->fetch_assoc($result))
		echo "\t\t\t\t\t\t\t\t".'<tr><td><input type="text" name="rank['.$cur_rank['id'].']" value="'.pun_htmlspecialchars($cur_rank['rank']).'" size="24" maxlength="50" /></td><td><input type="text" name="min_posts['.$cur_rank['id'].']" value="'.$cur_rank['min_posts'].'" size="7" maxlength="7" /></td><td><input type="submit" name="update['.$cur_rank['id'].']" value="Modifier" />&nbsp;<input type="submit" name="remove['.$cur_rank['id'].']" value="Supprimer" /></td></tr>'."\n";
...
<?php
	...
	echo "\t\t\t\t\t\t\t".'<p>Aucun rang dans la liste.</p>'."\n";
